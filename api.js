...
export const API = {
    ...
    async loadBook(name) {
        ...
        // [二改版移植] 强健的条目数据格式化与错误修复
        Object.entries(safeEntries).forEach(([uidKey, raw], idx) => {
            const entry = raw || {};
            const uidFallback = Number(uidKey);
            entry.uid = Number.isFinite(Number(entry.uid)) ? Number(entry.uid) : (Number.isFinite(uidFallback) ? uidFallback : idx);
    ...
    async getAllBoundBookNames(forceRefresh = false) {
        ...
        // [二改版移植] 恢复极速同步获取角色绑定，彻底抛弃导致严重卡顿的 fetch 请求
        characters.forEach((charItem) => {
            if (!charItem) return;
            // 兼容浅层与深层数据
            const dataFields = charItem.data || charItem;
            const curCharName = charItem.name || '未知角色';
