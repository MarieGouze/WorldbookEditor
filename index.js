...
jQuery(async () => {
    ...
    // 检查 ST 核心是否已完成初始化 (通过 eventSource 状态或数据可用性判断)
    // 如果 world_names 尚未加载，说明插件运行过早，需要等待 APP_READY
    if (typeof world_names === 'undefined' || !Array.isArray(world_names) || world_names.length === 0) {
        // 监听 APP_READY 事件，确保 getSettings 已完成
